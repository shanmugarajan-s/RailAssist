...
# Illustrative mapping of complaint category -> responsible department, used
# only to route/group complaints within this prototype's dashboards. This is
# an application-level interpretation for demo purposes, not an official
# Rail Madad / Indian Railways department routing table.
DEPARTMENT_MAPPING = {
    "Coach Facilities / Cleanliness": "Mechanical & Housekeeping Dept.",
    "Security / Lost Property": "RPF (Railway Protection Force)",
    "Medical Assistance": "Railway Medical Dept.",
    "Catering / Vendors": "IRCTC / Catering Dept.",
    "Train Delay / Status": "Operating Dept.",
    "Ticketing / Reservation": "Commercial Dept.",
    "Other / General Service": "General Administration",
    "Administrative Follow-up": "Administrative Dept.",
}
...
def determine_priority(category, complaint_text):
    """Shared priority rule used everywhere in the app (single complaint
    submission AND batch Dataset Check), so the two flows can never drift
    apart. Returns (priority, reason)."""
    base_high = category in ["Security / Lost Property", "Medical Assistance"]
    urgency_hit = check_urgency_signal(complaint_text)

    if base_high:
        return "HIGH", f"Category '{category}' is treated as high priority."
    elif urgency_hit:
        return "HIGH", f"Escalated on keyword match: '{urgency_hit}'."
    else:
        return "STANDARD", "No high-priority category or urgency keyword matched."
...
def analyze_complaint(complaint, artifacts):
    ...
    priority, priority_reason = determine_priority(category, complaint)
...
# ============================================================
    ...
    if st.button("🚀 Analyze & Submit Complaint", use_container_width=True):
        ...
        if not complaint.strip():
            ...
            with c2:
                st.metric("Department", result.get("department", "N/A"))
            ...
            st.caption(
                "Department is an illustrative routing based on the predicted "
                "category, not an official Rail Madad department mapping. "
...
# ============================================================
    ...
    if complaints_df.empty:
        ...
        with col_a:
            st.markdown("### Priority Split (High vs Standard)")
            priority_counts = (
        ...
        with col_b:
            if "Department" in complaints_df.columns:
                st.markdown("### Department-wise Complaint Count")
                dept_counts = (
                    complaints_df["Department"]
                    .value_counts()
        ...
        if "Department" in complaints_df.columns:
            st.markdown("### Department-wise Priority Breakdown")
            st.caption(
                "How many HIGH vs STANDARD priority complaints are pending in "
                "each department right now — useful for deciding where staff "
                "attention is needed most."
            )
            dept_priority = (
                complaints_df.groupby(["Department", "Priority"])
                .size()
                .unstack(fill_value=0)
            )
            for col in ["HIGH", "STANDARD"]:
                if col not in dept_priority.columns:
                    dept_priority[col] = 0
            dept_priority = dept_priority[["HIGH", "STANDARD"]].sort_values(
                "HIGH", ascending=False
            )
            st.dataframe(dept_priority, use_container_width=True)
        ...
        if "Department" in complaints_df.columns:
            queue_cols.append("Department")
        queue_cols += ["Priority", "Status"]

        st.dataframe(
            complaints_df[queue_cols].tail(10),
...
# ============================================================
    ...
    if not st.session_state.complaints:
        ...
        dept_options = ["All"] + sorted(
            complaints_df["Department"].dropna().unique().tolist()
        ) if "Department" in complaints_df.columns else ["All"]
        ...
        if department_filter != "All" and "Department" in filtered.columns:
        ...
        if not filtered.empty:
            ...
            if selected.get("Department"):
                st.write("**Department:**", selected["Department"])
...
# ============================================================
# DATASET CHECK (staff-side CSV upload + validation)
# ============================================================
elif portal == "👨‍💼 Staff Portal" and page == "📂 Dataset Check":

    st.markdown(
        '<div class="main-title">📂 Dataset Check</div>',
        unsafe_allow_html=True,
    )

    st.caption(
        "Upload a CSV file to check it against this prototype's expected format, "
        "and optionally run the current trained model on it. This does NOT "
        "retrain the model — it only validates/tests a file against the model "
        "that is already trained on train.csv."
    )

    uploaded_file = st.file_uploader("Upload a CSV file", type=["csv"])

    if uploaded_file is not None:
        # Try a couple of common encodings, same as the main training loader.
        new_df = None
        ...
        if new_df is None:
            st.error(
                "Could not read this file as CSV. Check that it's a valid, "
                "comma-separated .csv file."
            )
        else:
            st.success(f"File loaded: {new_df.shape[0]} rows, {new_df.shape[1]} columns.")

            st.markdown("### 1. Structure check")
            ...
            with c3:
                st.metric("Duplicate rows", int(new_df.duplicated().sum()))

            st.write("**Columns found:**", ", ".join(new_df.columns.astype(str)))

            expected_cols = {"Item ID", "Sentiment", "SentimentText"}
            found_cols = set(new_df.columns)
            missing_expected = expected_cols - found_cols

            if not missing_expected:
                st.success(
                    "This file matches the training format exactly "
                    "(Item ID, Sentiment, SentimentText)."
                )
            else:
                st.warning(
                    f"This file does not have all of the original training columns "
                    f"(missing: {', '.join(sorted(missing_expected))}). "
                    "You can still pick a text column below to run predictions on it."
                )
            ...
            st.markdown("### 4. Run the trained model on this file")

            text_col = st.selectbox(
                "Which column contains the complaint text?",
                options=list(new_df.columns),
                index=(
                    list(new_df.columns).index("SentimentText")
                    if "SentimentText" in new_df.columns
                    else 0
                ),
            )

            if st.button("▶️ Run predictions on this file"):
                model = artifacts["best_model"]
                working_df = new_df.copy()
                working_df[text_col] = working_df[text_col].astype(str)
                working_df["Clean_Text"] = working_df[text_col].apply(preprocess_text)

                n_empty = int((working_df["Clean_Text"].str.len() == 0).sum())
                if n_empty:
                    st.caption(
                        f"{n_empty} row(s) became empty after cleaning and will show "
                        "as 'Other / General Service' with low confidence."
                    )

                preds = model.predict(working_df["Clean_Text"])
                working_df["Predicted Category"] = [
                    CATEGORY_MAPPING.get(int(p), "Other / General Service")
                    for p in preds
                ]
                working_df["Department"] = working_df["Predicted Category"].map(
                    lambda c: DEPARTMENT_MAPPING.get(c, "General Administration")
                )
                working_df["Confidence"] = [
                    estimate_confidence(model, t) for t in working_df["Clean_Text"]
                ]

                priority_reason_pairs = [
                    determine_priority(cat, raw_text)
                    for cat, raw_text in zip(
                        working_df["Predicted Category"], working_df[text_col]
                    )
                ]
                working_df["Priority"] = [p for p, _ in priority_reason_pairs]
                working_df["Priority Reason"] = [r for _, r in priority_reason_pairs]

                st.markdown("#### Predicted category distribution")
                st.bar_chart(working_df["Predicted Category"].value_counts())

                col_p, col_d = st.columns(2)
                with col_p:
                    st.markdown("#### Priority split (High vs Standard)")
                    st.bar_chart(working_df["Priority"].value_counts())
                with col_d:
                    st.markdown("#### Department-wise count")
                    st.bar_chart(working_df["Department"].value_counts())

                st.markdown("#### Department-wise priority breakdown")
                dept_priority_table = (
                    working_df.groupby(["Department", "Priority"])
                    .size()
                    .unstack(fill_value=0)
                )
                for col in ["HIGH", "STANDARD"]:
                    if col not in dept_priority_table.columns:
                        dept_priority_table[col] = 0
                dept_priority_table = dept_priority_table[
                    ["HIGH", "STANDARD"]
                ].sort_values("HIGH", ascending=False)
                st.dataframe(dept_priority_table, use_container_width=True)

                # If this file also has true labels (Sentiment column), do a
                # quick sanity check of the current model against it. This is
                # NOT retraining — just checking how the existing model performs
                # on this new file.
                if "Sentiment" in working_df.columns:
                    try:
                        y_true = working_df["Sentiment"].astype(int)
                        acc = accuracy_score(y_true, preds)
                        f1 = f1_score(y_true, preds, average="macro")
                        st.markdown("#### Quick check against labels in this file")
                        cc1, cc2 = st.columns(2)
                        with cc1:
                            st.metric("Accuracy on this file", f"{acc:.2%}")
                        with cc2:
                            st.metric("Macro F1 on this file", f"{f1:.2%}")
                        st.caption(
                            "This checks how the model (already trained on "
                            "train.csv) performs on this uploaded file — it does "
                            "not retrain or update the model."
                        )
                    except Exception:
                        st.caption(
                            "Found a 'Sentiment' column but couldn't score it "
                            "against predictions (unexpected values)."
                        )
                ...
                st.download_button(
                    ...
                    data=csv_bytes,
                    file_name="railassist_predictions.csv",
                    mime="text/csv",
                )
